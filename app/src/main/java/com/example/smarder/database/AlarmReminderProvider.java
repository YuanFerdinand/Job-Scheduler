package com.example.smarder.database;
...
public class AlarmReminderProvider extends ContentProvider {
    ...
    private int updateReminder(Uri uri, ContentValues contentValues, String selection, String[] selectionArgs) {
        ...
        SQLiteDatabase database = mDbHelper.getWritableDatabase();
        int rowsUpdated = database.update(AlarmReminderContract.AlarmReminderEntry.TABLE_NAME, contentValues, selection, selectionArgs);
        if (rowsUpdated != 0) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
        return rowsUpdated;
